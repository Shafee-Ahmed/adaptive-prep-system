...
async def run_iteration(
    ...
    questions_by_section = await generate_use_case.execute(
        ...
        num_questions_per_section=3  # 3 per section for faster testing
    )
    ...
    # 2. Simulate answers (use partial strategy for adaptation)
    if iteration_num == 1:
        strategy = "partial"
    elif iteration_num == 2:
        strategy = "weak_focused"  # Focus on weak topics from iter1
    else:
        strategy = "partial"
    
    user_answers = simulate_answers(all_questions, strategy=strategy)
    
    # 3. Submit answers and get session
...
async def main():
    ...
    print("="*60)
    for r in results:
        print(f"Iteration {r['iteration']}: Sections {r['sections']} - Score: {r['score']:.1f}% - Weak topics: {r['weak_topics_count']}")
