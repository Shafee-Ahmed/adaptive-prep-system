...
class SQLiteKBRepository(KBRepository):
        ...
        try:
            with self._get_connection() as conn:
                ...
                for result in session.answer_results:
                    ...
                    # Update topic performance if wrong answer
                    if not result.is_correct and result.topic:
                        self._update_topic_performance(conn, result.section_id, result.topic)
            
            logger.info(f"Session {session.id} saved with {len(session.answer_results)} answers")
        except Exception as e:
            raise DatabaseError(f"Failed to save session: {e}")
    
    def _update_topic_performance(self, conn: sqlite3.Connection, section_id: int, topic: str) -> None:
        ...
        try:
            with self._get_connection() as conn:
                ...
                for row in rows:
                    ...
                    for a in answers:
                        session.answer_results.append(AnswerResult(
    ...
    def get_mastered_topics(self, section_ids: List[int]) -> List[str]:
        ...
        try:
            with self._get_connection() as conn:
                # Find topics that appear in correct answers but never in wrong answers
                rows = conn.execute(
                    f"""
